...
## 使用方式二：直接打开 Obsidian Vault

也可以直接把 Obsidian 的 vault 文件夹作为 Codex 项目打开，然后让 skill 在当前 vault 里原地新建或更新笔记。

适合这种情况：

- 你希望生成的笔记直接出现在 Obsidian 里；
- 你不想先生成到 `./vault` 再复制进去；
- 你之后要对某一章、某一节或某个文件做增量更新；
- 你已经有标签、双链、文件夹顺序或手写笔记，不希望被整体重建。

调用示例：

```text
请使用 $book-to-obsidian，以当前项目作为 Obsidian vault。
读取我拖进来的对话 Markdown 和教材，直接在当前 vault 中新建或更新笔记。
不要创建额外的 vault 子目录，不要删除或重命名我已有的笔记。
```

这个模式下，skill 会把当前工作区根目录当成 Obsidian vault 根目录。如果检测到 `.obsidian/`，会优先进入这种模式。

更新已有文件时，原则是：

- 先读取原文件；
- 保留已有 front matter、标签、别名和双链；
- 只更新和当前章节/小节匹配的生成内容；
- 不删除、不重命名你的手写笔记；
- 不确定是否覆盖时，生成候选稿或更新日志。
